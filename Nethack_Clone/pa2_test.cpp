...
int main(int argc, char * argv[])
{
	...
	if (argc > 1) {runCount = atoi(argv[1]);}

	for (int i = 0; i < runCount; i++)
	{
		DungeonLevel test(1);	
		bool passed = unitTest(test);

		if (passed) {cout << endl << "Dungeon Level has passed the test!" << endl;}
		else {cout << endl << "Dungeon Level has failed the test!" << endl;}
	}
